...
export default function Notifications() {
	...
	const fetchNotifications = (page) => {
		setLoading(true)
		axios.get(`${process.env.NEXT_PUBLIC_HOST}/api/admin/notifications?userID=${user?.userID}&page=${page}`)
			.then(res => {
	...
	const handleClearNotifications = () => {
		...
		axios.delete(`${process.env.NEXT_PUBLIC_HOST}/api/admin/notifications/clear-all?userID=${user?.userID}`)
			.then(res => {
				const { status, data } = res
				if (status === 203) {
					setNotifications([])
